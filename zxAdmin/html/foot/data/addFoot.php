...
if(in_array($arrname[1], $fileType)){
    ...
    if($flag){//如果图片上传成功，则将数据加入数据库中
        $footName=trim($_REQUEST['footName']);
        $classify=trim($_REQUEST['classify']);
        $footNum=trim($_REQUEST['footNum']);
        $footPrice=trim($_REQUEST['footPrice']);
       
        $link=new PDO("mysql:host=localhost;dbname=zx__ordsystem","root","");
        $link->query("set names utf8");
        $result=$link->query("INSERT INTO zx_foots(footName,classify,footNum,footPrice,imgsrc,path) VALUES('$footName','$classify','$footNum','$footPrice','$filename','$newfn')");          
