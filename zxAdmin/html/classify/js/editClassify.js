$.get('./data/showClassify.php',function(data){
    var grid_data = data;
    ...
    jQuery(grid_selector).jqGrid({
        ...
        datatype: "local", //数据来源
        height: 370,    //高度
        colNames:['菜品类别', '类别简介'], //表头名称，列显示名称，是一个数组对象
        colModel:[
            {name:'classify',index:'classify', width:1,align:'center',editable: true,editoptions:{size:"80",maxlength:"30"}},
            {name:'blurb',index:'blurb',width:4, editable:true,edittype:'text',editoptions:{size:"80"},sortable:false}
            ], 
        ...
        editurl: "./data/editClassify.php",//保存位置
        //caption: "我的预定详情查看",
        autowidth: true,
    });
    ...
    jQuery(grid_selector).jqGrid('navGrid',pager_selector,
        ...
        {//edit record form编辑框
            ...
            beforeSubmit: function(postData, formid){//向服务器传送参数
                var editData = {"flag": 1}; //额外新增参数
                postData = $.extend(postData, editData);//拼接参数上传
                return[true,"success"];   
            }, 
            afterSubmit: function(data, postData){//从服务器的返回值
                switch(data.responseText){
                    case '1':
                        return [false,"该分类已存在",postData.id];
                    case '2':
                        setTimeout(function () { 
                            window.location.reload();
                        }, 1000);//编辑成功1s后刷新
                        return [false,"分类编辑成功",postData.id];
                    case '3':
                        return [false,"该分类已存在,请重新输入！",postData.id];
                    default:
                        return [false,"请检查您的网络！",postData.id];
                }
            }
        },
        {//new record form
        ...
        {//delete record form删除框
            ...
            top: 100,
            recreateForm: true,
            url:'./data/delClassify.php',//替代原来的editurl
            beforeShowForm : function(e) {
                var form = $(e[0]);
            ...
            afterSubmit: function(data, postData){//从服务器的返回值    
                switch(data.responseText){
                    case '1':
                        ...
                        return [false,"该分类删除成功",postData.id];
                    case '2':
                        return [false,"该分类删除失败！",postData.id];
                    default:
                        return [false,"请检查您的网络！",postData.id];
                }
            }
