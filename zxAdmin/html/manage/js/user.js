$.post('./data/editUser.php',{flag:1},function(data){
    var grid_data = data;
    jQuery(function($) {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        datatype: "local", //数据来源
        height: 370,    //高度
        colNames:['用户名', '联系方式'], //表头名称，列显示名称，是一个数组对象
        colModel:[
            {name:'loginName',index:'loginName', width:1,align:'center',editable: true,editoptions:{size:"80",maxlength:"30"}},
            {name:'userTel',index:'userTel',width:1, editable:true,editoptions:{size:"80",maxlength:"11"}}
            ], 
        ...
        editurl: "./data/editUser.php",//保存位置
        autowidth: true,
    });
            ...
            afterSubmit: function(data, postData){//从服务器的返回值
                switch(data.responseText){
                    case '1':
                        ...
                        return [false,"该用户信息的资料已经更新！",postData.id];
                    case '2':
                        return [false,"该用户信息的资料更新失败!",postData.id];
                    default:
            ...
            afterSubmit: function(data, postData){//从服务器的返回值
                switch(data.responseText){
                    case '1':
                        ...
                        return [false,"该用户信息的资料已经删除！",postData.id];
                    case '2':
                        return [false,"该用户信息的资料删除失败!",postData.id];
                    default:
